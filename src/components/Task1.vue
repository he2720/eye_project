  ...
  <div>
    <div  v-loading="loading"  element-loading-text="拼命加载中">
      ...
      <a href="javascript:;" class="file"
        >点击上传文件
        <input :id="index" type="file" multiple @change="markDocument" /> </a
      >
      <el-button type="info" plain id="upload-btn1" @click="getMarkDocument"
        >查看已提交标注文件</el-button>
    </div>
    <br />
      ...
      <el-tabs v-model="task.activeName">
        ...
        <el-tab-pane label="标注详情" name="second">
          <el-card shadow="always">
            ...
            <el-divider></el-divider>
            人员：<el-tag v-for="(peo, index) in task.person" :key="index+'peo'">
              {{ peo.username }} </el-tag
            >
<!--            <el-divider></el-divider>-->
            <div v-show="false">
...
<!--            <el-divider></el-divider>-->
        ...
        <el-tab-pane label="项目管理" name="fourth" v-if="false">
          <el-card>
            人员分配：
            <el-table :data="task.person" stripe style="width: 100%" border>
              ...
              <el-table-column prop="marks" label="标注类型" align="center" v-if="radio==6">
                <template slot-scope="scope">
                  <el-tag
                    size="medium"
   ...
   <el-dialog title="图片上传中" :visible.sync="loading"></el-dialog>
    <el-dialog title="新建项目" :visible.sync="dialogVisible">
      <el-form :rules="rules" :model="project" ref="ruleForm">
        <el-form-item prop="name" label="项目名称"><el-input
...
<!--      标注类型<br /><br />-->
        <h4>任务类型：</h4>
        <el-radio-group v-model="project.type" @change="showAttris">
          <el-radio :label="3">图像标签</el-radio>
          <el-radio :label="4">真菌性角膜炎标注</el-radio>
          <el-radio :label="5">多眼病标注</el-radio>
          <el-radio :label="6">图像识别</el-radio>
          <el-radio :label="9">图像分割</el-radio>
        </el-radio-group>
        <br>
<!--      全局标注-->
      <el-button style="margin-top: 10px ;margin-bottom: 10px" size="small" @click="setAttri" v-show="project.type==3"
        >设置属性</el-button>
        <div v-show="project.type==4||project.type==5||project.type==3">
          <div  v-for="(item,index) in showattris" :key="index+'attris'" v-if="index!=0">
           {{item['name']}} :<el-tag v-for="(item1,index1) in item['opts']" :key="index1+'attris1'" style="margin-left: 10px">{{item1}}</el-tag>
          </div>

        </div>
<!--      局部标注-->
      <el-checkbox-group v-model="project.marks" v-show="false" style="margin-top: 10px ;margin-bottom: 10px">
    ...
    <el-dialog title="编辑项目" :visible.sync="dialogVisible1">
      <el-form :rules="rules" :model="editTask" ref="ruleForm">
        <el-form-item disabled required label="项目名称">
...
<!--        <el-form-item prop="director" label="负责人"><el-input-->
<!--        v-model="editTask.director"-->
<!--        placeholder="请输入内容"-->
<!--        style="width: 222px"-->
<!--      ></el-input>-->
<!--        </el-form-item>-->
        <el-form-item label="当前已上传图片数量" v-show="false"><el-input-number disabled v-model="editTask.number"></el-input-number
        ></el-form-item>
        <el-form-item label="描述" style="margin-left: 10px"><el-input
...
<!--      <br>-->
      ...
      <br /><br />
        ...
        <h4>任务类型：</h4>
        <el-radio-group v-model="protype" disabled>
          <el-radio :label="3">图像标签</el-radio>
          <el-radio :label="4">真菌性角膜炎</el-radio>
          <el-radio :label="5">多眼病预测</el-radio>
          <el-radio :label="6">图像识别</el-radio>
          <el-radio :label="9">图像分割</el-radio>
        </el-radio-group><br>
<!--      标注类型<br /><br />-->
<!--      全局标注-->
      <el-button style="margin: 2px" size="small" @click="setAttri" v-show="protype==3"
        >设置属性</el-button>
<!--      局部标注-->
        <div v-show="protype==4||protype==5||protype==3">
          <div  v-for="(item,index) in editTask.attris" :key="index+'attris'" v-if="index!=0">
            {{item['name']}}<el-tag v-for="(item1,index1) in item['opts']" :key="index1+'attris1'" style="margin-left: 10px">{{item1}}</el-tag>
          </div>

        </div>
        <el-checkbox-group v-model="editTask.marks" v-show="false">
    ...
    <el-dialog title="已上传图片" :visible.sync="dialogVisibleDocument">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="lastDocument">上一张</el-button>
        <el-button type="primary" @click="nextDocument">下一张</el-button>
      </span>
    </el-dialog>

<!--    v-for="(peo,index) in tasks[activeTask].person"-->
...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        director:[{required: true, message: '请输入负责人姓名', trigger: 'blur'}],
  ...
  methods: {
    ...
    showAttris(){
      if(this.project.type==5){
        this.showattris=[{'name': '', 'opts': []}, {'name': '患病类别', 'opts': ['青光眼', '正常', '糖尿病型视网膜病变1期', '糖尿病型视网膜病变2期', '糖尿病型视网膜病变3期', '糖尿病型视网膜病变4期', '年龄性黄斑病变早期', '年龄性黄斑病变晚期']}]
      }
      else if(this.project.type==4)
      {
        this.showattris=[{'name': '', 'opts': []}, {'name': '真菌性角膜炎', 'opts': ['阴性', '阳性']}]
      }
      else if(this.project.type==3)
      {
        this.showattris=this.project.attris
      }
    },
    send(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          let data1 = [];
          if(this.project.type==5){
            this.project.attris=[{'name': '', 'opts': []}, {'name': '患病类别', 'opts': ['青光眼', '正常', '糖尿病型视网膜病变1期', '糖尿病型视网膜病变2期', '糖尿病型视网膜病变3期', '糖尿病型视网膜病变4期', '年龄性黄斑病变早期', '年龄性黄斑病变晚期']}]
          }
          else if(this.project.type==4)
          {
            this.project.attris=[{'name': '', 'opts': []}, {'name': '真菌性角膜炎', 'opts': ['阴性', '阳性']}]
          }
    ...
    editPro(index) {
      ...
      this.protype = this.tasks[index].type
      ...
      this.$confirm("此操作将永久删除该项目, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          this.$axios({
            ...
            else if (response.data == "无法删除")
            {
              that.$message.error("不能删除示例项目！");
            }
              else {
    ...
    markDocument() {
      ...
      console.log(this.tasks[this.activeTask]);
      this.files = file.files;
      let that = this;
      let name = this.tasks[this.activeTask].name;
      let type = this.tasks[this.activeTask].type;
      let attris = this.tasks[this.activeTask].attris[1]
      var fd = new FormData();
    ...
    getMarkDocument(){
      ...
      this.$axios({
        ...
        this.markedDocument = this.markedDocuments[0].filename;
    ...
    nextDocument(){
      ...
      this.number1++;
      this.markedDocument = this.markedDocuments[this.number1].filename;
      this.$forceUpdate();
